...
| Neon production project/branch | Production | CAT relational data, audit events, protected PII envelopes and indexes | CAT + Neon | Protected branch `production`; scoped app/migration/backup roles; provider evidence recorded privately | Verified distinct from DocLinks and Preview; 22 migrations applied |
| Neon preview branch/project | Preview | Synthetic CAT data | CAT + Neon | Branch `develoment`; distinct non-admin `local801_preview_app` | Verified distinct from Production and DocLinks |
...
| Cloudflare R2 CAT recovery bucket | Production recovery | Database dumps/checksums and encrypted object recovery copies | CAT + Cloudflare | Backup/recovery workflows and runs 32597930819, 32598092186, 32642686698 | Private configuration and database/object/key recovery verified |
...
| `cat.cyang.io` DNS/TLS | Production | Public application hostname and TLS termination | CAT + DNS operator + Vercel/Cloudflare | Vercel domain, deployment alias and response-header evidence | Live HTTP 200, TLS/HSTS/CSP/no-store verified; final authenticated launch smoke pending |
| Microsoft Entra ID | Production | OIDC identity, verified email, MFA assurance | Organization + Microsoft + CAT | `LOCAL801_OIDC_*`; owner-confirmed three-account acceptance | Production client/MFA/account lifecycle accepted; private admin inventory remains required |
...
| GitHub security services | Repository | Dependency, secret, and code findings | CAT + GitHub | Dependabot, CodeQL workflow, branch/security settings | CAT checks protected; zero CAT dependency/open CodeQL/secret findings |
| Sentry plus Vercel/GitHub native logs | Runtime | Redacted application errors plus provider-native request/firewall/workflow metadata | CAT + providers | CAT Sentry project, alert rule, Vercel logs, GitHub Actions | Redacted Sentry delivery and two high-priority email triggers verified; retention approval pending |
