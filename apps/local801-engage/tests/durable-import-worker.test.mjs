...
import { acceptDurablePreviewCsv } from "../src/lib/import-async-acceptance.ts";
...
test("starter uses the durable Preview gate", async () => {
  ...
  const query = async () => [{
    ...
    workflow_run_id: null, processing_version: "local801-import-v1", source_count: 1, csv_source_count: 1,
  }];
...
test("starter fails closed for missing, ambiguous, non-CSV, or non-queued sources", async () => {
  ...
  for (const patch of [
    { source_count: 0, csv_source_count: 0 },
    { source_count: 2, csv_source_count: 2 },
    { source_count: 1, csv_source_count: 0 },
...
test("strictly synthetic durable Preview acceptance succeeds", async () => {
  const statements = [];
  const file = new File([
  ...
  const result = await acceptDurablePreviewCsv({
...
test("durable Preview acceptance rejects non-synthetic identities", async () => {
  let storageCalls = 0;
  let workflowCalls = 0;
  const file = new File([
    "Local #,Employee ID,Work Email\n801,SYNTH-E-1,one@example.com",
  ], "real-identity.csv", { type: "text/csv" });
  await assert.rejects(acceptDurablePreviewCsv({
    actor: {
      organizationId,
...
test("upload route retains same-origin/auth/workspace protection and async mode is explicit", async () => {
  ...
  assert.match(route, /form\.get\("processingMode"\) === "durable_preview"/);
  assert.match(route, /status: 202/);
  assert.match(form, /defaultValue="synchronous"/);
});
