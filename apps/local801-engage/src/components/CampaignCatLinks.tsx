...
async function mutation(url: string, method: "POST" | "DELETE", body?: Record<string, unknown>) {
  ...
  if (!response.ok) throw new Error(payload.message || "CAT couldn’t update this link.");
}
...
export function CampaignCatLinks({ campaignHandle, links, actions }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "CAT couldn’t link the action."); }
    finally { setPending(false); }
  }

  async function remove(linkHandle: string) {
    if (!window.confirm("Unlink this campaign from the CAT Action? CAT will keep the audit history.")) return;
    setPending(true); setMessage("");
    try { await mutation(`/api/campaign-cat-links/${linkHandle}`, "DELETE"); setMessage("CAT Action unlinked from this campaign."); router.refresh(); }
  ...
  return <div className="grid">
    ...
    </li>)}</ul> : <p className="muted">This campaign is not linked to a CAT Action yet.</p>}
