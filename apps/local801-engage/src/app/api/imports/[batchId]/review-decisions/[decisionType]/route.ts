...
function logFailure(operation: "set" | "clear", error: unknown) {
  const source = error && typeof error === "object" ? error as Record<string, unknown> : {};
  console.error("[import-review-decision] request failed", {
    operation,
...
async function contextFor(request: Request, params: Promise<{ batchId: string; decisionType: string }>) {
  ...
  try { await enforceWorkspaceRateLimit(context, "import"); }
  catch (error) { if (error instanceof RateLimitError) return { response: rateLimitResponse(error) }; throw error; }
...
export async function PUT(request: Request, { params }: { params: Promise<{ batchId: string; decisionType: string }> }) {
  ...
  } catch (error) { logFailure("set", error); return unavailable(); }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ batchId: string; decisionType: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: true }, { headers: noStore });
  } catch (error) { logFailure("clear", error); return unavailable(); }
}
