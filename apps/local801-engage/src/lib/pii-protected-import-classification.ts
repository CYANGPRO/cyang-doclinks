...
export const PROTECTED_IMPORT_REVIEW_CLASSIFICATION_CTE = `
  ...
  import_indexes AS (
    ...
    WHERE index.organization_id = $1
      ...
      AND index.index_domain IN (
        ...
        'contact:work-email','identifier:employee-identifier','identifier:member-identifier',
        'contact:personal-email','contact:work-phone','contact:cell-phone','contact:home-phone'
      )
  ),
  live_evidence AS (
  ...
  direct_field_matches AS (
    SELECT row.import_row_id,
      EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = $1 AND live.entity_type = 'person'
          AND live.entity_id = match.person_id
          AND live.index_domain = 'person:first-name'
          AND live.index_key_version = imported.index_key_version
          AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id
          AND imported.index_domain = 'person:first-name'
      ) AS first_name_matches,
      EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = $1 AND live.entity_type = 'person'
          AND live.entity_id = match.person_id
          AND live.index_domain = 'person:last-name'
          AND live.index_key_version = imported.index_key_version
          AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id
          AND imported.index_domain = 'person:last-name'
      ) AS last_name_matches,
      EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = $1 AND live.entity_type = 'person'
          AND live.entity_id = match.person_id
          AND live.index_domain = 'person:preferred-name'
          AND live.index_key_version = imported.index_key_version
          AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id
          AND imported.index_domain = 'person:preferred-name'
      ) AS preferred_name_matches,
      EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.person_contact_methods contact
          ON contact.organization_id = $1 AND contact.person_id = match.person_id
          AND contact.contact_type = 'work_email' AND contact.is_primary = true
          AND contact.archived_at IS NULL
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = contact.organization_id
          AND live.entity_type = 'person_contact_method'
          AND live.entity_id = contact.id
          AND live.index_domain = 'contact:work-email'
          AND live.index_key_version = imported.index_key_version
          AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id
          AND imported.index_domain = 'contact:work-email'
      ) AS primary_work_email_matches
      , EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.person_contact_methods contact
          ON contact.organization_id = $1 AND contact.person_id = match.person_id
        ...
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = contact.organization_id
          AND live.entity_type = 'person_contact_method' AND live.entity_id = contact.id
          AND live.index_domain = 'contact:personal-email'
          AND live.index_key_version = imported.index_key_version AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id AND imported.index_domain = 'contact:personal-email'
      ) AS home_email_matches
      , EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.person_contact_methods contact
          ON contact.organization_id = $1 AND contact.person_id = match.person_id
          AND contact.contact_type = 'phone' AND contact.contact_label = 'work' AND contact.archived_at IS NULL
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = contact.organization_id
          AND live.entity_type = 'person_contact_method' AND live.entity_id = contact.id
          AND live.index_domain = 'contact:work-phone'
          AND live.index_key_version = imported.index_key_version AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id AND imported.index_domain = 'contact:work-phone'
      ) AS work_phone_matches
      , EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.person_contact_methods contact
          ON contact.organization_id = $1 AND contact.person_id = match.person_id
          AND contact.contact_type = 'phone' AND contact.contact_label = 'cell' AND contact.archived_at IS NULL
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = contact.organization_id
          AND live.entity_type = 'person_contact_method' AND live.entity_id = contact.id
          AND live.index_domain = 'contact:cell-phone'
          AND live.index_key_version = imported.index_key_version AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id AND imported.index_domain = 'contact:cell-phone'
      ) AS cell_phone_matches
      , EXISTS (
        SELECT 1 FROM import_indexes imported
        JOIN local801.person_contact_methods contact
          ON contact.organization_id = $1 AND contact.person_id = match.person_id
          AND contact.contact_type = 'phone' AND contact.contact_label = 'home' AND contact.archived_at IS NULL
        JOIN local801.pii_exact_indexes live
          ON live.organization_id = contact.organization_id
          AND live.entity_type = 'person_contact_method' AND live.entity_id = contact.id
          AND live.index_domain = 'contact:home-phone'
          AND live.index_key_version = imported.index_key_version AND live.index_hash = imported.index_hash
        WHERE imported.import_row_id = row.import_row_id AND imported.index_domain = 'contact:home-phone'
      ) AS home_phone_matches
    FROM batch_rows row
  ...
  row_facts AS (
    SELECT row.*, COALESCE(match.person_count, 0) AS person_count, match.person_id,
      ...
      COALESCE(direct.home_email_matches, false) AS home_email_matches,
      COALESCE(direct.work_phone_matches, false) AS work_phone_matches,
      COALESCE(direct.cell_phone_matches, false) AS cell_phone_matches,
      COALESCE(direct.home_phone_matches, false) AS home_phone_matches,
  ...
  categorized AS (
    SELECT facts.*,
      CASE
        ...
        WHEN facts.row_hash IS NULL OR facts.direct_pii_field_set_version < 2
        ...
        WHEN (facts.direct_pii_presence_mask & 1) = 0
          ...
          OR ((facts.direct_pii_presence_mask & 16) = 0
            ...
            AND (facts.direct_pii_presence_mask & 8) = 0) THEN 'needs_attention'
        ...
        WHEN facts.person_count = 1 AND (
          ...
          OR ((facts.direct_pii_presence_mask & 64) <> 0 AND NOT facts.home_email_matches)
          OR ((facts.direct_pii_presence_mask & 128) <> 0 AND NOT facts.work_phone_matches)
          OR ((facts.direct_pii_presence_mask & 256) <> 0 AND NOT facts.cell_phone_matches)
          OR ((facts.direct_pii_presence_mask & 512) <> 0 AND NOT facts.home_phone_matches)
