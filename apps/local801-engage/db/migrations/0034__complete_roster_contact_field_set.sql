...
alter table local801.import_row_pii
  add constraint import_row_pii_field_masks_ck check (
    ...
    or (direct_pii_field_set_version = 2
    ...
    or (direct_pii_field_set_version = 3
      and direct_pii_presence_mask between 0 and 127
      and direct_pii_validity_mask between 0 and 127
      and (direct_pii_validity_mask & direct_pii_presence_mask) = direct_pii_validity_mask)
    or (direct_pii_field_set_version = 4
      and direct_pii_presence_mask between 0 and 255
      and direct_pii_validity_mask between 0 and 255
      and (direct_pii_validity_mask & direct_pii_presence_mask) = direct_pii_validity_mask)
    or (direct_pii_field_set_version = 5
...
begin
  ...
  if new.normalized_json ?| array[
    'first_name','last_name','preferred_name','work_email','personal_email','home_email',
    'work_phone','cell_phone','home_phone','employee_identifier','member_identifier'
  ] then
  ...
  if not exists (
    ...
    where protected.organization_id = new.organization_id and protected.import_row_id = new.id
      and protected.direct_pii_field_set_version in (2, 3, 4, 5)
